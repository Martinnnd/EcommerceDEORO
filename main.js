...
componentes.forEach((el) => {
    if (!el.enStock ) {
        mensajeComponentes += `
          ...
          <button id="add-cart" value=${el.id} class="btn btn-secondary" disabled>Agregar al carrito</button>
        </div>
        `
        ...
        <button id="add-cart" value=${el.id} class="btn btn-secondary">Agregar al carrito</button>
      </div>
        `
    };
});
...
cards.forEach((el, idx) => {
  if (el.className === "card") {
      el.onclick = () => agregarAlCarrito(idx);
  };
});
